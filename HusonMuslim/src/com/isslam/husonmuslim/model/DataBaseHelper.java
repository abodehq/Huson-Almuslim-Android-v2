...
import com.isslam.husonmuslim.DBActivity;
...
public class DataBaseHelper extends SQLiteOpenHelper {
	...
	public void creatVirtualDB() {
		Log.e("myDataBase", myDataBase + "");

		SQLiteDatabase db = this.getReadableDatabase();
		db.execSQL("CREATE VIRTUAL TABLE  vcontents USING fts3(_id INTEGER PRIMARY KEY, content TEXT,chapter_id TEXT ,semi_chapter_id INTEGER ,vname TEXT,footer TEXT,search_tags TEXT  );");
	...
	public void copyVirtual() {
		Log.e("copyVirtual", "copyVirtual");
		SQLiteDatabase db = this.getReadableDatabase();
		db.execSQL("insert into vcontents select contents._id,contents.content ,contents.chapter_id,contents.semi_chapter_id,semi_chapters.name,footer,search_tags from contents,semi_chapters WHERE contents.semi_chapter_id=semi_chapters._id ;");
		Log.e("start Copy", "virtual");
	}
			...
			String content_id)// /Get SemiChapter Content
	{
		...
		// Select All Query
		String selectQuery = "SELECT contents._id,contents.content,contents.chapter_id,contents.semi_chapter_id,chapters.name,semi_chapters.name,footer,repeat FROM contents,chapters,semi_chapters where contents._id = "
				+ content_id
		...
		if (cursor.moveToFirst()) {
			do {
				...
				map.put("footer", cursor.getString(6));
				map.put("repeat", cursor.getString(7));
	...
	public ArrayList<HashMap<String, String>> getSearch(String value) {
		...
		// Select All Query
		String selectQuery = "SELECT  vcontents._id,vcontents.content,vcontents.chapter_id,vcontents.semi_chapter_id,chapters.name,semi_chapters.name,footer FROM vcontents,chapters,semi_chapters WHERE vcontents MATCH '*"
				+ value
	...
	public ArrayList<HashMap<String, String>> delete_note_by_content_id(
			String content_id) {
		SQLiteDatabase db = this.getReadableDatabase();
		db.delete("notes", "content_id=" + content_id, null);
		db.close();
		return get_semi_chapters_notes();
	}

	public void insert_note(String content_id, String title, String body,
	...
	public ArrayList<HashMap<String, String>> get_semi_chapters_notes()// get
	// allsemichapters
	{
		ArrayList<HashMap<String, String>> contentList = new ArrayList<HashMap<String, String>>();

		// Select All Query
		String selectQuery = "SELECT semi_chapters._id,semi_chapters.name,semi_chapters.chapter_id,notes._id,title,body,color,date,content,contents._id FROM notes,semi_chapters,contents where notes.content_id = contents._id AND semi_chapters._id = contents.semi_chapter_id ";
		SQLiteDatabase db = this.getReadableDatabase();
		...
		if (cursor.moveToFirst()) {
			do {
				...
				map.put("chapter_id", cursor.getString(2));
				map.put("chapter_name", "");
				map.put("note_id", cursor.getString(3));
				map.put("title", cursor.getString(4));
				map.put("body", cursor.getString(5));
				map.put("color", cursor.getString(6));
				map.put("date", cursor.getString(7));
				map.put("content", cursor.getString(8));
				map.put("content_id", cursor.getString(9));
				contentList.add(map);
			} while (cursor.moveToNext());
		}
		cursor.close();
	...
	public void delete_all_notes()// get
	// allsemichapters
	{
		SQLiteDatabase db = this.getReadableDatabase();
	...
	public boolean get_notes_is_empty() {
		boolean result = false;

		// Select All Query
		String selectQuery = "SELECT notes._id FROM notes  ";
		SQLiteDatabase db = this.getReadableDatabase();
